...
export function SettingsPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Account Profile */}
      <Panel className="space-y-4 border-l-4 border-cyan-500 bg-gradient-to-r from-slate-900 via-cyan-950/20 to-slate-900">
        <div>
          <h3 className="text-xl font-semibold text-white flex items-center gap-2">👤 Account Profile</h3>
          <p className="mt-1 text-sm text-slate-400">Manage your FlowTrack Pro cloud account.</p>
        </div>
        <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
          ...
          <button
            onClick={async () => {
              ...
              try {
                await signOut(auth);
                // The global route guard will instantly kick them back to LoginPage
              } catch (error: any) {
                showMessage(`Sign out failed: ${error.message}`);
              }
            }}
            className="rounded-2xl border border-white/20 bg-slate-800 px-6 py-2.5 font-bold text-white shadow-lg hover:bg-rose-500/80 hover:border-rose-400 hover:text-white transition-all"
          >
            Sign Out
          </button>
        </div>
        <div className="text-xs text-slate-400 mt-2 flex items-center gap-2">
          <div className={`h-2 w-2 rounded-full ${cloudSyncStatus === 'error' ? 'bg-rose-500' : cloudSyncStatus === 'syncing' ? 'bg-amber-500 animate-pulse' : 'bg-emerald-500'}`} />
          ...
          <div className="flex flex-wrap gap-3">
            <button
              onClick={async () => {
                ...
                if (!user) return;
                const data = await fs.fetchUserData(user.uid);
                const settingsList = Object.entries(data.settings).map(([k, v]) => ({ key: k, value: typeof v === 'string' ? v : JSON.stringify(v) }));
                exportData({
                  app: "FlowTrack",
            ...
            <input
              ...
              onChange={(event) => {
                ...
                void importBackup(file).then((payload) => {
                  void importAll(payload.subjects, payload.sessions, payload.settings).then(() => {
          ...
          <button
            onClick={async () => {
              ...
              try {
                const user = useAppStore.getState().user;
                if (user) {
                  await fs.clearAllSubjects(user.uid);
                  await fs.clearAllSessions(user.uid);
                  await fs.clearAllSettings(user.uid);
                }
