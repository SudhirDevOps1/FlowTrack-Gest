...
function getProviderEndpoint(provider: AiConfig["provider"], _model: string, ollamaUrl: string) {
  switch (provider) {
...
export function AIAssistantPage() {
  ...
  useEffect(() => {
    ...
    if (user) {
      fs.fetchUserData(user.uid).then(data => {
        const val = { value: data.settings["ai_token_usage"] };
        if (val && val.value) {
          try {
            setTokenUsage(JSON.parse(val.value as string));
          } catch {}
        }
      });
    }
  ...
  const recordTokenUsage = useCallback(async (prompt: number, completion: number, total: number) => {
    setTokenUsage(prev => {
      ...
      const user = useAppStore.getState().user;
      if (user) void fs.saveSetting(user.uid, "ai_token_usage", JSON.stringify(next));
      return next;
    });
  }, []);
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      if (provider === "gemini") {
        ...
        const endpoint = getProviderEndpoint(provider, model, ollamaUrl);
        const headers = getProviderHeaders(provider, apiKey);
        const defaultModel = provider === "ollama" ? "llama3" : provider === "groq" ? "llama-3.3-70b-versatile" : "gpt-4o-mini";
        ...
        setConnectionResult({ success: true, message: `Successfully connected to ${PROVIDERS.find(p => p.id === provider)?.name}!` });
      }
    } catch (e: any) {
        ...
        {(showConfig || window.innerWidth >= 1280) && (
          ...
          >
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => {
                    ...
                    setTokenUsage(zero);
                    const user = useAppStore.getState().user;
                    if (user) void fs.saveSetting(user.uid, "ai_token_usage", JSON.stringify(zero));
                  }}
            ...
            <Panel className="border border-emerald-500/10 bg-emerald-950/5 p-4 space-y-2">
              ...
              <p className="text-xs text-slate-400 leading-relaxed">
                All AI credentials and keys are strictly saved securely to your private cloud storage. Study logs are never transmitted anywhere except to your configured API endpoints. Select **Local AI Rules** or **Ollama** for 100% private offline operations.
              </p>
            </Panel>
          </motion.div>
        )}
