...
export interface AppState {
  ...
  initApp: (uid: string) => Promise<void>;
  ...
  importAll: (subjects: Subject[], sessions: StudySession[], settings?: AppSettings[]) => Promise<void>;
...
async function saveTimer(uid: string, timer: TimerSnapshot): Promise<void> {
  await fs.saveSetting(uid, "timer", JSON.stringify(timer));
}
...
function generateIntegrityHash(id: string, actualSeconds: number, status: string): string {
  if (typeof btoa !== 'undefined') {
    return btoa(`${id}:${actualSeconds}:${status}:flowtrack_secure_v1`);
  }
  return "";
}

function clampElapsedSeconds(_session: StudySession | undefined, currentElapsed: number): number {
  return Math.max(0, Math.floor(currentElapsed));
}
...
export const useAppStore = create<AppState>()((set: any, get: any) => ({
  ...
  initApp: async (uid: string) => {
    const userData = await fs.fetchUserData(uid);
    const subjects = userData.subjects || [];
    const sessions = userData.sessions || [];
    const settings = userData.settings || {};
    
    const timerSetting = settings["timer"] ? { value: settings["timer"] } : undefined;
    const pomodoroSetting = settings["pomodoroMode"] ? { value: settings["pomodoroMode"] } : undefined;
    const goalSetting = settings["dailyGoalHours"] ? { value: settings["dailyGoalHours"] } : undefined;
    const strictFocusSetting = settings["strictFocusMode"] ? { value: settings["strictFocusMode"] } : undefined;
    const weeklyTargetSetting = settings["weeklyTargetHours"] ? { value: settings["weeklyTargetHours"] } : undefined;
    const focusMusicSetting = settings["focusMusicEnabled"] ? { value: settings["focusMusicEnabled"] } : undefined;
    const notificationsSetting = settings["notificationsEnabled"] ? { value: settings["notificationsEnabled"] } : undefined;
    const keyboardShortcutsSetting = settings["keyboardShortcutsEnabled"] ? { value: settings["keyboardShortcutsEnabled"] } : undefined;
    const themeSetting = settings["theme"] ? { value: settings["theme"] } : undefined;
    const achievementsSetting = settings["achievements"] ? { value: settings["achievements"] } : undefined;
    const dailyGoalHitStreakSetting = settings["dailyGoalHitStreak"] ? { value: settings["dailyGoalHitStreak"] } : undefined;
    const autoPauseOnHiddenSetting = settings["autoPauseOnHidden"] ? { value: settings["autoPauseOnHidden"] } : undefined;
    const profileSetting = settings["user_profile"] ? { value: settings["user_profile"] } : undefined;
    const aiConfigSetting = settings["ai_config"] ? { value: settings["ai_config"] } : undefined;
    const autoCarryForwardSetting = settings["autoCarryForward"] ? { value: settings["autoCarryForward"] } : undefined;
      ...
      if (todaySessions.length === 0) {
        ...
        if (pastSessions.length > 0) {
          ...
          if (clonedList.length > 0) {
            await fs.bulkSaveSessions(get().user?.uid || "", clonedList);
    ...
    if (subjects.length === 0 && finalSessions.length === 0) {
      ...
      await fs.bulkSaveSubjects(get().user?.uid || "", seedSubjects);
      await fs.bulkSaveSessions(get().user?.uid || "", seedSessions);
      set({ 
    ...
    if (parsedTimer && parsedTimer.activeSessionId && !parsedTimer.isPaused) {
      const activeSession = finalSessions.find((s: any) => s.id === parsedTimer.activeSessionId);
      if (activeSession) {
        ...
        await fs.saveSetting(get().user?.uid || "", "timer", JSON.stringify(safeTimer));
        await fs.updateSession(get().user?.uid || "", parsedTimer.activeSessionId, {
          status: "paused",
  ...
  createSubject: async (name: string, color: string, emoji?: string, weeklyGoalMinutes?: number) => {
    ...
    await fs.saveSubject(get().user?.uid || "", newSubject);
  ...
  updateSubject: async (id: string, name: string, color: string, emoji?: string, weeklyGoalMinutes?: number) => {
    // update subject
    const updatedSubject = { ...get().subjects.find((s: any) => s.id === id)!,  name, color, emoji, weeklyGoalMinutes  };
    await fs.saveSubject(get().user?.uid || "", updatedSubject);
    set((state: AppState) => ({
  ...
  deleteSubject: async (id: string) => {
    await fs.deleteSubject(get().user?.uid || "", id);
    /* TODO: manually delete sessions by subject id */
    ...
    if (timer.activeSessionId) {
      ...
      if (activeSession?.subjectId === id) {
        await saveTimer(get().user?.uid || "", defaultTimer);
  ...
  createSession: async (input: CreateSessionInput) => {
    ...
    await fs.saveSession(get().user?.uid || "", session);
    set((state: AppState) => {
  ...
  updateSession: async (session: StudySession) => {
    ...
    await fs.saveSession(get().user?.uid || "", updated);
    ...
    if (timer.activeSessionId === updated.id) {
      ...
      await saveTimer(get().user?.uid || "", nextTimer);
  ...
  deleteSession: async (id: string) => {
    await fs.deleteSession(get().user?.uid || "", id);
    const timer = get().timer;
  ...
  importAll: async (subjects: Subject[], sessions: StudySession[], settings?: { key: string; value: string }[]) => {
    const uid = get().user?.uid || "";
    await fs.clearAllSubjects(uid);
    await fs.clearAllSessions(uid);
    await fs.bulkSaveSubjects(uid, subjects);
    await fs.bulkSaveSessions(uid, sessions);
    if (settings && settings.length > 0) {
      await fs.clearAllSettings(uid);
      for (const s of settings) {
        await fs.saveSetting(uid, s.key, s.value);
      }
    }
    if (uid) await get().initApp(uid);
  },
  ...
  addManualEntry: async (input: { subjectId: string; date: string; hours: number; notes?: string; tags?: string[] }) => {
    ...
    const actualSeconds = Math.floor(input.hours * 3600);
    const session: StudySession = {
      id: sessionId,
      subjectId: input.subjectId,
      ...
      updatedAt: new Date().toISOString(),
      manualEntry: true,
      integrityHash: generateIntegrityHash(sessionId, actualSeconds, "completed"),
    };
    await fs.saveSession(get().user?.uid || "", session);
  ...
  startSession: async (sessionId: string) => {
    ...
    await saveTimer(get().user?.uid || "", timer);
    await fs.updateSession(get().user?.uid || "", session.id, { 
      status: "in_progress", 
      updatedAt: new Date().toISOString(),
      integrityHash: generateIntegrityHash(session.id, session.actualSeconds, "in_progress")
    });
      set((state: AppState) => {
        const newSessions: StudySession[] = state.sessions.map((item: StudySession) => (item.id === session.id ? { 
          ...item, 
  ...
  pauseSession: async () => {
    ...
    await saveTimer(get().user?.uid || "", nextTimer);
    await fs.updateSession(get().user?.uid || "", timer.activeSessionId, {
      status: "paused",
      actualSeconds: nextAccumulated,
      updatedAt: new Date().toISOString(),
      integrityHash: generateIntegrityHash(timer.activeSessionId, nextAccumulated, "paused"),
    });
    set((state: AppState) => {
  ...
  resumeSession: async () => {
    ...
    await fs.updateSession(get().user?.uid || "", timer.activeSessionId, { 
      status: "in_progress", 
      updatedAt: new Date().toISOString(),
      integrityHash: generateIntegrityHash(timer.activeSessionId, timer.accumulatedSeconds, "in_progress")
    });
    set((state: AppState) => {
      const newSessions: StudySession[] = state.sessions.map((session: StudySession) =>
        session.id === timer.activeSessionId ? { 
  ...
  stopSession: async () => {
    ...
    await fs.updateSession(get().user?.uid || "", timer.activeSessionId, {
      ...
      updatedAt: new Date().toISOString(),
      integrityHash: generateIntegrityHash(timer.activeSessionId, elapsed, finalStatus),
    });
    await saveTimer(get().user?.uid || "", defaultTimer);
    ...
    const newSessions: StudySession[] = get().sessions.map((session: StudySession) =>
      session.id === timer.activeSessionId
        ? { 
            ...session, 
            actualSeconds: elapsed, 
            status: finalStatus as SessionStatus, 
            updatedAt: new Date().toISOString(),
            integrityHash: generateIntegrityHash(timer.activeSessionId, elapsed, finalStatus)
          }
        : session
  ...
  syncActiveSession: async (nowMs?: number) => {
    ...
    await fs.updateSession(get().user?.uid || "", timer.activeSessionId, { 
      ...
      updatedAt: new Date().toISOString(),
      integrityHash: generateIntegrityHash(timer.activeSessionId, clampedElapsed, activeSession.status),
    });
  ...
  setPomodoroMode: (enabled: boolean) => {
    void fs.saveSetting(get().user?.uid || "", "pomodoroMode", String(enabled));
    set({ pomodoroMode: enabled });
  },

  setStrictFocusMode: (enabled: boolean) => {
    void fs.saveSetting(get().user?.uid || "", "strictFocusMode", String(enabled));
    set({ strictFocusMode: enabled });
  },

  setAutoPauseOnHidden: (enabled: boolean) => {
    void fs.saveSetting(get().user?.uid || "", "autoPauseOnHidden", String(enabled));
    set({ autoPauseOnHidden: enabled });
  },

  setDailyGoalHours: (hours: number) => {
    void fs.saveSetting(get().user?.uid || "", "dailyGoalHours", String(hours));
    set({ dailyGoalHours: hours });
  },

  setWeeklyTargetHours: (hours: number) => {
    void fs.saveSetting(get().user?.uid || "", "weeklyTargetHours", String(hours));
    set({ weeklyTargetHours: hours });
  },

  setFocusMusicEnabled: (enabled: boolean) => {
    void fs.saveSetting(get().user?.uid || "", "focusMusicEnabled", String(enabled));
    set({ focusMusicEnabled: enabled });
  },

  setNotificationsEnabled: (enabled: boolean) => {
    void fs.saveSetting(get().user?.uid || "", "notificationsEnabled", String(enabled));
    set({ notificationsEnabled: enabled });
  },

  setKeyboardShortcutsEnabled: (enabled: boolean) => {
    void fs.saveSetting(get().user?.uid || "", "keyboardShortcutsEnabled", String(enabled));
    set({ keyboardShortcutsEnabled: enabled });
  },

  setTheme: (theme: ThemeName) => {
    void fs.saveSetting(get().user?.uid || "", "theme", theme);
  ...
  setDailyGoalHitStreak: (streak: number) => {
    void fs.saveSetting(get().user?.uid || "", "dailyGoalHitStreak", String(streak));
  ...
  cloneSession: async (sessionId: string, targetDate: string) => {
    ...
    await fs.saveSession(get().user?.uid || "", clonedSession);
    set((state: AppState) => {
  ...
  createRecurringSessions: async (sessionId: string, config: RecurrenceConfig) => {
    ...
    if (newSessions.length > 0) {
      await fs.bulkSaveSessions(get().user?.uid || "", newSessions);
  ...
  setUserProfile: async (profile: UserProfile) => {
    await fs.saveSetting(get().user?.uid || "", "user_profile", JSON.stringify(profile));
    set({ profile });
  },
  ...
  setAiConfig: async (config: AiConfig) => {
    await fs.saveSetting(get().user?.uid || "", "ai_config", JSON.stringify(config));
    set({ aiConfig: config });
  },

  setAutoCarryForward: (enabled: boolean) => {
    void fs.saveSetting(get().user?.uid || "", "autoCarryForward", String(enabled));
  ...
  updateDailyGoalStreak: async (newSessions: StudySession[]) => {
    ...
    await fs.saveSetting(get().user?.uid || "", "dailyGoalHitStreak", String(streak));
    set({ dailyGoalHitStreak: streak });
