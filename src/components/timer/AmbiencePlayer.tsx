...
export function AmbiencePlayer() {
  ...
  useEffect(() => {
    ...
    if (user) {
      void fs.fetchUserData(user.uid).then(data => {
        const setting = { value: data.settings["ambience_playlist"] };

        if (setting && setting.value) {
          try {
            let parsed = JSON.parse(setting.value as string);
            ...
            if (hasBrokenLinks) {
              ...
              void fs.saveSetting(user.uid, "ambience_playlist", JSON.stringify(parsed));
            }
  ...
  const savePlaylistToDb = async (list: typeof savedPlaylist) => {
    ...
    const user = useAppStore.getState().user;
    if (user) await fs.saveSetting(user.uid, "ambience_playlist", JSON.stringify(list));
  };
